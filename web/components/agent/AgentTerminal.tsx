...
export function AgentTerminal({ speed }: AgentTerminalProps) {
  ...
  const runDemo = useCallback(async (scenario: Scenario) => {
    ...
    try {
      ...
      try {
        const repRes = await facilitatorFetch(`/api/reputation/${operatorAddress}`);
      ...
      const orderRes = await facilitatorFetch("/api/orders", {
        method: "POST",
        body: JSON.stringify({
      ...
      try {
        const cdRes = await facilitatorFetch(`/api/orders/${orderData.id}/confirm-delivery`, { method: "POST" });
      ...
      if (scenario === "happy") {
        ...
        const disputeRes = await facilitatorFetch(`/api/disputes/${orderData.id}`, {
          method: "POST",
          body: JSON.stringify({
        ...
        const resolveRes = await facilitatorFetch(`/api/disputes/${disputeData.disputeId}/resolve`, {
          method: "POST",
          body: JSON.stringify({
            buyerPct: 70,
      ...
      try {
        const [bRes, sRes] = await Promise.all([
          facilitatorFetch(`/api/reputation/${address}?fresh=true`),
        ...
        <div className="panel-surface space-y-2 rounded-xl border border-accent/20 bg-accent/5 px-4 py-3">
          {[
            ...
            <div key={hash} className="flex items-center gap-2 text-sm">
              ...
              <TxLink hash={hash} label={`${hash.slice(0, 22)}...`} className="text-sm" />
            </div>
          ))}
        </div>
      )}
